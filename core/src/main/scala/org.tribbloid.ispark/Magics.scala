...
abstract class Magic[T](val name: Symbol, parser: MagicParsers[T]) {
    def apply(interpreter: SparkInterpreter, input: String) = {
...
object Magic {
    val magics = List(TypeMagic, ResetMagic)
...
object ResetMagic extends EmptyMagic('resetSpark) {
    def handle(interpreter: SparkInterpreter) {
        interpreter.resetSpark()
    }
}
